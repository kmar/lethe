...
struct LETHE_API FallbackAlignedAlloc
{
	...
	static void *Realloc(void *ptr, size_t newSize, size_t align);
	// deallocation
...
{
	// allocate block
	void *(*Alloc)(size_t size, size_t align) = nullptr;
	...
	void *(*Realloc)(void *ptr, size_t newSize, size_t align) = nullptr;
	// free block
	void (*Free)(void *ptr) = nullptr;

	inline void *CallAlloc(size_t size, size_t align)
	{
		return Alloc ? Alloc(size, align) : FallbackAlignedAlloc::Alloc(size, align);
	}

	inline void *CallRealloc(void *ptr, size_t newSize, size_t align)
	{
		return Realloc ? Realloc(ptr, newSize, align) : CallAlloc(newSize, align);
	}
